...
function setColor(){
	...
	var p1dec = p1.value.replace("#","");
	var r1 = parseInt(p1dec.substring(0,2), 16);
    var g1 = parseInt(p1dec.substring(2,4), 16);
    var b1 = parseInt(p1dec.substring(4,6), 16);
  
	colors = [r1/255, g1/255, b1/255, a];
  
	var uColorLocation = gl.getUniformLocation(shaderProgram, "uColor");
	gl.uniform4fv(uColorLocation, colors);
}

function setSize(){
	var slider_n = document.getElementById("slider");
	var slider_out = document.getElementById("slider_output");
	
	slider_out.value = slider_n.value;
	var uPointSizeLocation = gl.getUniformLocation(shaderProgram, "uPointSize");
	gl.uniform1f(uPointSizeLocation, parseInt(slider_n.value));
}